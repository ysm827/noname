...
const skills = {
  ...
  pothuilv: {
    ...
    async cost(event, trigger, player) {
      ...
      const result = await player.chooseControl(list).set("prompt", get.prompt(event.skill, target)).set("prompt2", "令其从牌堆中获得一张此颜色的牌").set("ai", () => {
    ...
    async content(event, trigger, player) {
      ...
      if (card) {
        ...
        player.when({ global: "phaseUseEnd" }).filter((evt) => evt == trigger).step(async (event2, trigger2, player2) => {
          ...
          if (bool2) {
            ...
            if (cards2.length && game.hasPlayer((current) => current != target)) {
              ...
              if (result?.bool && result.targets?.length) {
                ...
                player.line(target2);
                const gainEvent = target2.gain(cards2, "give");
